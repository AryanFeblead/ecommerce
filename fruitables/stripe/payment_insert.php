...
if ( empty( $error ) ) {
    ...
    if ( !empty( $payment ) && $payment->status == 'succeeded' ) {
        ...
        $date = date( 'y-m-d' );
        mysqli_query( $conn, "INSERT INTO `stripe_payment`( `fullname`, `email`, `item_description`, `currency`, `amount`, `transaction_id`, `payment_status`, `created_at`)VALUES ('$fullname','$email ',' $item_description','$currency','$amount','$transaction_id','$payment_status','$date')" );
